...
- **Blueprints**: community-shared construction diagrams and blueprints for builds. Filterable by free or pay-to-build, style, and tags. Builder profiles aggregate all their blueprints. Data sourced from a Discord forum channel (`BLUEPRINT_CHANNEL_FORUM_ID`).
...
- **Bug reports**: community-tracked bugs in Where Winds Meet building features. Data sourced from a shared Google Spreadsheet (`data/bugs.json`), filterable by severity and platform. Includes a "how to report" guide and links to official report channels.
- **Updates**: per-version changelog of building features and mechanics, curated by the community from a shared Google Spreadsheet (`data/patches.json`). Organized by game version.
